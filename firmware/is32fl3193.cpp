...
void rgb_set_auto(bool enable, bool led)
{
    ...
    const uint8_t current_driver_address_value[2] = {0x00, current_driver_settings[led]}; // current enable address, curren enable setting
...
void rgb_set_breathing(bool breathing)
{
    ...
    const uint8_t breathing_address_value[2] = {0x02, breathing_settings[breathing]}; // breathing address, breathing setting
...
void rgb_init()
{
    ...
    i2c_write_blocking(I2C_PORT, IS31_ADDR, (const uint8_t[2]){0x03, 0x08}, 2, false);
}
